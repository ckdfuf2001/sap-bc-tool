"""SM50 (Work Processes of AS Instance) RFC-only 재현. 분석서: docs/analysis/A4H_SM50.md
프로그램 RSMON000_ALV_NEW (TSTC 실측, DYPNO 1000, AUTH S_ADMI_FCD 실측).
RFC 경로 TH_WPINFO (FMODE=R 실측). 시그니처 실측: IMP MAX_ELEMS/SRVNAME/WITH_CPU/WITH_MTX_INFO, TAB WPLIST.

외부 MCP 래핑용: from sap_tcode_sm50 import TOOLS, call_tool
"""
import os
...
# --- F01: 워크 프로세스 목록 (원천: TH_WPINFO) ---
def f01_wp_info(conn, *, with_cpu=""):
    """워크 프로세스 목록. Args: with_cpu(선택, 'X' 시 CPU 정보).
    Returns: {"wp": [...]}}"""
    params = {}
    if with_cpu:
        params["WITH_CPU"] = with_cpu
    res = conn.call("TH_WPINFO", **params)
    wp = res.get("WPLIST", [])
    return {"result": {"wp": wp, "count": len(wp)}, "raw": wp}


TOOLS = [
    {"id": "F01", "name": "f01_wp_info", "description": "SM50 워크 프로세스 목록 (TH_WPINFO)",
     "input_schema": {"type": "object",
        "properties": {
            "with_cpu": {"type": "string", "description": "'X' 시 CPU 정보, 선택"}},
        "required": []},
     "func": "f01_wp_info"},
]
...
if __name__ == "__main__":
    import sys
    import json
    # 사용법: python sap_tcode_sm50.py A4H F01 --params '{}'
